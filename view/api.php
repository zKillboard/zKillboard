...
function handler($request, $response, $args, $container) {
    ...
    try {
        ...
        if ($queryString != '') {
            ...
            return $response->withStatus(403)->withHeader('Cache-Tag', 'www,api,error');
        }

        if ($redis->get("zkb:reinforced") == true) {
            $response->getBody()->write('503 Reinforced mode, please try again later');
            return $response->withStatus(503)->withHeader('Cache-Tag', 'www,api,error');
        }
    ...
    foreach ($return as $json) {
        ...
        if (isset($queryParams['callback']) && Util::isValidCallback($queryParams['callback'])) {
            ...
            return $response->withHeader('Content-Type', 'application/javascript; charset=utf-8')->withHeader('Cache-Tag', 'www,api');
        } else {
            // JSON output
            ...
            $response->getBody()->write($output);
            return $response->withHeader('Content-Type', 'application/json; charset=utf-8')->withHeader('Cache-Tag', 'www,api');
        }
    } catch (Exception $ex) {
        ...
        return $response->withHeader('Content-Type', 'application/json; charset=utf-8')->withHeader('Cache-Tag', 'www,api,error');
    }
}
