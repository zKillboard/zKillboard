...
function handler($request, $response, $args, $container)
{
	...
	if ($kvc->get('zkb:noapi') == 'true') {
		return $container->get('view')->render($response->withHeader('Cache-Tag', 'www,error,login'), 'error.pug', ['message' => 'Downtime is not a good time to login, the CCP servers are not reliable, sorry.']);
	}
	...
	try {
		...
		try {
			...
			return $container->get('view')->render($response->withHeader('Cache-Tag', 'www,error,login'), 'error.pug', ['message' => 'CCP SSO is currently having issues, please wait a moment and try again...']);
		}
		...
		if ($e->getMessage() == 'Invalid state returned - possible hijacking attempt') {
			if ($_SESSION['characterID'] > 0) {
				...
				return $container->get('view')->render($response->withStatus(503)->withHeader('Cache-Tag', 'www,error,login'), 'error.pug', ['message' => "Please try logging in again, but don't double/triple click this time. CCP's login form isn't very good at handling multiple clicks... "]);
			}
		} elseif ($e->getMessage() == 'Undefined array key "access_token"') {
			return $container->get('view')->render($response->withStatus(503)->withHeader('Cache-Tag', 'www,error,login'), 'error.pug', ['message' => 'CCP failed to send access token data, please try logging in again.']);
		} else {
			Util::zout(print_r($e, true));
			return $container->get('view')->render($response->withStatus(503)->withHeader('Cache-Tag', 'www,error,login'), 'error.pug', ['message' => $e->getMessage()]);
		}
	} finally {
		sem_release($sem);
	}
