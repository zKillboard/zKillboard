...
function handler($request, $response, $args, $container) {
    ...
    try {
        ...
        if ($state !== $sessionState) {
            return $container->get('view')->render($response->withHeader('Cache-Tag', 'error,patreon'), "error.pug", ['message' => "Something went wrong with security. Please try again."]);
        }
        ...
        return $container->get('view')->render($response->withHeader('Cache-Tag', 'error,patreon'), "error.pug", ['message' => "Something went wrong with the login from Patreon's end, sorry, can you please try logging in again? *"]);
    }
}
