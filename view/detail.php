...
while ($mdb->count("queueInfo", ['killID' => $id])) sleep(1);

$exists = $mdb->exists("killmails", ['killID' => $id]);
if (!$exists) return $app->render("404.html", array("message" => "KillID $id does not exist."), 404);
...
if ($details == null)
{
	...
	if ($killdata["victim"]["groupID"] == 29)
	{
		...
		if ($relatedKill) $relatedShip = ['killID' => $relatedKill["killID"], 'shipTypeID' => $relatedKill["involved"][0]["shipTypeID"]];
	} else
	{
