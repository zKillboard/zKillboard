...
$extra['droppableisk'] = isset($killdata['info']['zkb']['totalDroppableValue']) ? $killdata['info']['zkb']['totalDroppableValue'] : 0;
$extra['droppedisk'] = isset($killdata['info']['zkb']['droppedValue']) ? $killdata['info']['zkb']['droppedValue'] : droppedIsk(md5($id), $killdata['items']);
$extra['shipprice'] = Price::getItemPrice($killdata['victim']['shipTypeID'], date('Y-m-d H:i', strtotime($killdata['info']['dttm'])));
$extra['destroyediskWship'] = isset($killdata['info']['zkb']['destroyedValue']) ? $killdata['info']['zkb']['destroyedValue'] : $extra['shipprice'] + destroyedIsk(md5($id), $killdata['items']);
$extra['destroyedisk'] = max(0, $extra['destroyediskWship'] - $extra['shipprice']);
$extra['droppabledroppedisk'] = min($extra['droppedisk'], $extra['droppableisk']);
$extra['droppabledestroyedisk'] = max(0, $extra['droppableisk'] - $extra['droppabledroppedisk']);
...
$extra['droppableprice'] = Util::iskToUsdEurGbp($extra['droppableisk']);
...
$details['comments'] = array_values($comments);
    ...
    if ($pageview == 'remaining') {
        return $container->get('view')->render($response->withHeader('Cache-Tag', "www,kill,kill:$id,kill:remaining"), "components/attackers_list.pug", [
	...
	if ($pageview == 'items') {
        return $container->get('view')->render($response->withHeader('Cache-Tag', "www,kill,kill:$id,kill:items"), "components/item_list.pug", $details);
    }

    return $container->get('view')->render($response->withHeader("Cache-Tag", "www,detail,kill,kill:$id"), 'detail.pug', $details);
}
