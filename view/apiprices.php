...
function handler($request, $response, $args, $container) {
    ...
    if (isset($queryParams['callback']) && Util::isValidCallback($queryParams['callback'])) {
        ...
        $response = $response->withHeader('Content-Type', 'application/javascript; charset=utf-8')
                            ...
                            ->withHeader('Cache-Tag', "www,api,prices,type:$id");
        $response->getBody()->write($content);
        return $response;
    } else {
        // Handle JSON output
        $response = $response->withHeader('Content-Type', 'application/json; charset=utf-8')->withHeader('Cache-Tag', "www,api,prices,type:$id");
