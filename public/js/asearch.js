...
function toggleRollingTime(event, enabled) {
	...
	$('#rolling-times').removeClass('btn-secondary').removeClass('btn-primary');
...
function getHTML(suggestion) {
	...
	var data = $("<span>")
		.addClass("entity")
		.addClass('btn')
		.css({ display: "inline-flex", alignItems: "center", justifyContent: "flex-start", textAlign: "left", flex: "1 1 auto", width: "auto", minWidth: 0, padding: "4px 10px 4px 0", borderRadius: 0, color: "#ddd", backgroundColor: "#111" })
	...
	return $("<div>")
		...
		.css({ display: "flex", alignItems: "stretch", borderRadius: "7px", overflow: "hidden", padding: 0, backgroundColor: "#111" })
		.addClass('filter')
...
function toggleFilterBtn() {
	var element = $(this);
	if (element.hasClass('btn-primary')) element.removeClass('btn-primary').addClass('btn-secondary');
	else element.removeClass('btn-secondary').addClass('btn-primary');
	updateAsearchButtonStates();
	clickPage1();
}
...
function toggleFiltersClick() {
	...
	if (has_primary) element.removeClass('btn-primary').addClass('btn-secondary');
	else element.removeClass('btn-secondary').addClass('btn-primary');
	updateAsearchButtonStates();
	toggleFilters();
	if (allowChange) setHash();
...
async function btn_save(event) {
	...
	try {
		...
		try {
			...
			button.addClass('btn-info').attr("title", "Saved URL copied");
			showToast('Saved URL copied to clipboard');
		} catch (err) {
			console.error("Failed to copy:", err);
			button.addClass('btn-info').attr("title", "Saved URL ready");
