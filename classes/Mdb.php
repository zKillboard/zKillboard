...
class Mdb
{
	...
	public function getDb($attempt = 0)
	{
		try 
		{
			if ($this->mongoClient == null) $this->mongoClient = new MongoClient();
			if ($this->db == null) $this->db = $this->mongoClient->selectDB("zkillboard");
	...
	public function find($collection, $query = [], $sort = [], $limit = null, $includes = [])
	{
		...
		if (php_sapi_name() == 'cli') $cursor->timeout(-1);
		else $cursor->timeout(35000);

		// Set the sort and limit...
		if (sizeof($sort)) $cursor->sort($sort);
		if ($limit != null) $cursor->limit($limit);
		$result = iterator_to_array($cursor);
		$time = $timer->stop();
		//if ($time > 1000) Log::log("Long query (${time}ms): $serialized");

		if ($cacheTime > 0 && sizeof($result) > 0) Cache::set($cacheKey, $result, $cacheTime);
	...
	public static function group($collection, $keys = [], $query = [], $count = [], $sum = [], $sort = [], $limit = null)
	{
		...
		$pipeline = [];
		if (sizeof($query)) $pipeline[] = [ '$match' => $query];

		// Create the group by using the given key(s)
		$ids = [];
		foreach ($keys as $key=>$value)
		{
			if (is_numeric($key)) $ids[] = '$' . $value;
		...
		if (sizeof($count) == 0 && sizeof($sum) == 0) $group['count'] = [ '$sum' => 1];

		// Include counts in the $group
		if (!is_array($count)) $count = [$count];
		foreach($count as $s) $group[str_replace(".", "_", $s) . 'Count'] = [ '$sum' => 1];

		// Include sums in the $group
		if (!is_array($sum)) $sum = [$sum];
		foreach($sum as $s) $group[str_replace(".", "_", $s) . 'Sum'] = [ '$sum' => '$' . $s];
		...
		foreach ($keys as $key=>$value)
		{
			if (is_numeric($key)) $project[$value] = '$_id';
			else $project[$key] = '$_id.' . $key;
		}
		if (sizeof($count) == 0 && sizeof($sum) == 0) $project['count'] = 1;
		if (sizeof($count) > 0) foreach($count as $s) $project[str_replace(".", "_", $s) . 'Count'] = 1;
		if (sizeof($sum) > 0) foreach($sum as $s) $project[str_replace(".", "_", $s) . 'Sum'] = 1;
		...
		if (!$debug) MongoCursor::$timeout = -1; // this should be deprecated but aggregate doesn't have a timeout
		// Execute the query
