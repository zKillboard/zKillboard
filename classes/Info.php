...
class Info
{
	...
	public static function loadIntoRedis($type, $id)
	{
		...
		switch ($type) {
			case 'solarSystemID':
				...
				if ($starInfo != null)
					$data['sunTypeID'] = $starInfo['type_id'];
				if ($starInfo == null || $starInfo['type_id'] == null)
					$data['sunTypeID'] = 3802;
	...
	public static function getInfo($type, $id)
	{
		...
		if ($data == null) {
			...
			if ($data == null)
				$data = self::loadIntoRedis($type, $id);
	...
	public static function getRegionInfoFromSystemID($systemID)
	{
		...
		try {
			if (!is_array($data))
				$data = ['solarSystemID' => $systemID, 'name' => "System $systemID"];
	...
	public static function addInfo(&$element)
	{
		...
		foreach ($element as $key => $value) {
			...
			if ($class == 'MongoId' || $class == 'MongoDate' || $class == 'MongoDB\BSON\UTCDateTime' || $class == 'MongoDB\BSON\ObjectId' || $class == '__PHP_Incomplete_Class') {
				continue;
			}
			if (is_array($value)) {
				$element[$key] = self::addInfo($value);
			} elseif (!is_object($value) && $value != 0) {
				switch ($key) {
					case 'lastChecked':
	...
	public static function getLocationID($solarSystemID, $position)
	{
		...
		foreach ($systemLocations as $row) {
			...
			$hasWarpPoint = (
				isset($row['WarpX']) &&
				is_numeric($row['WarpX']) &&
				isset($row['WarpY']) &&
				is_numeric($row['WarpY']) &&
				isset($row['WarpZ']) &&
				is_numeric($row['WarpZ'])
			);
			...
			// Stargates and stations: subtract stored radius so kills at the structure surface compare correctly
			if (($row['type'] ?? '') === 'mapStargates' || ($row['type'] ?? '') === 'npcStations') {
				$structureRadius = (isset($row['Radius']) && is_numeric($row['Radius'])) ? (float) $row['Radius'] : 0.0;
