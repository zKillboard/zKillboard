...
class Price
{
	...
	protected static function doPopulateRareItemPrices($todaysLookup)
	{
		...
		if (sizeof($motherships) == 0) exit("no motherships, bailing"); // Haven't loaded all data yet, bail!
		foreach ($motherships as $mothership) {
			$typeID = $mothership["typeid"];
			if ($typeID == 3514) continue;
		...
		if (sizeof($titans) == 0) exit("no titans, bailing"); // Haven't loaded all data yet, bail!
