...
class MongoFilter
{
	public static function getKills($parameters)
	{
		...
		if ($limit > 200) $limit = 200;
		if ($limit < 1) $limit = 1;
		if ($page > 0) $cursor->skip($page * $limit);
		$cursor->limit($limit);

		$result = array();
		foreach($cursor as $row) $result[] = $row;
	...
	public static function buildQuery(&$parameters, $useElemMatch = true)
	{
		...
		{
			if (is_array($value)) $filter = ['$in' => $value];
			else $filter = $value;
			switch ($key)
			{
				case "week":
				...
				case "year":
					...
					if ($endKillID > 0) $and[] = ['killID' => [ '$lte' => $endKillID]];
					break;
				case "month":
					...
					$endKillID = self::getKillIDFromTime($end, -1);
					$and[] = ['killID' => [ '$gte' => $startKillID]];
					if ($endKillID > 0) $and[] = ['killID' => [ '$lt' => $endKillID]];
					break;
				case "relatedTime":
				...
				case "kills":
					...
					if ($useElemMatch) $elemMatch["isVictim"] = false;
					else $and[] = ["involved.isVictim" => false];
					break;
				case "losses":
					if ($useElemMatch) $elemMatch["isVictim"] = true;
					else $and[] = ["involved.isVictim" => true];
					break;
				case "finalblow-only":
					if ($useElemMatch) $elemMatch["finalBlow"] = true;
					else $and[] = ["involved.finalBlow" => true];
					break;
				case "allianceID":
				case "characterID":
				case "corporationID":
				...
				case "shipTypeID":
				case "isVictim":
					if ($useElemMatch) $elemMatch[$key] = $filter;
					else $and[] = ["involved." . $key => $filter];
					break;
				case 'regionID':
				case "solarSystemID":
		...
		if (sizeof($elemMatch) > 0) $and[] = ['involved' => ['$elemMatch' => $elemMatch]];

		// Prep the query, not using $and if it isn't needed
		$query = array();
		if (sizeof($and) == 1) $query = $and[0];
		else if (sizeof($and) > 1) $query = ['$and' => $and];
