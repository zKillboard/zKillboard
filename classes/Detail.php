...
subs:
	...
	public static function combineditems($md5, $items)
	{
		...
		foreach($items as $itm)
		{
			...
			for ($i = 0; $i <= 1; $i++) {
				$mItem = $itm;
				if ($i == 0) @$mItem["quantityDropped"] = 0;
				if ($i == 1) @$mItem["quantityDestroyed"] = 0;
				if (@$mItem["quantityDropped"] == 0 && @$mItem["quantityDestroyed"] == 0) continue;
        ...
        public static function fullCombinedItems($md5, $items)
        {
			...
			else $itemList[$key]["quantityDropped"] += $mItem["quantityDropped"];
