...
while (!Util::exitNow() && $timer->stop() < 115000)
{
	...
	if (!$unprocessed->hasNext()) sleep(1);
...
if ($debug && $counter > 0) Util::out("Added " . number_format($counter, 0) . " Kills.");
...
function validKill(&$kill)
{
	...
	foreach ($kill["attackers"] as $attacker) {
		if (isset($attacker["shipType"]["id"]))
		{
			...
			if ($attackerGroupID == 99) return true; // A sentry gun is involved
		}

		if (isset($attacker["shipType"]["id"]) && $attacker["shipType"]["id"] == 34495) return true; // A drifter is involved

		// Don't process the kill if it's NPC only
		if (isset($attacker["corporation"]["id"]) && $attacker["corporation"]["id"] == 1000125) return true;
