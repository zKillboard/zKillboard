...
while ($hour == date('H')) {
	...
	foreach ($redisQueues as $queue => $v) {
		...
		else if ($queueType == Redis::REDIS_ZSET) $queueCount = $redis->zCard($queue);
		else $queueCount = 0;
...
addInfo('', 0);
	...
	addInfo('', 0, false);
	addInfo('Character Scopes to check', (int) $redis->get('zkb:charKillmailScopesPending'), false);
	addInfo('Character Scopes', (int) $redis->get('zkb:charKillmailScopesTotal'), false);
	addInfo('Corporation Scopes to check', (int) $redis->get("zkb:corpKillmailScopesPending"), false);
	addInfo('Corporation Scopes', (int) $redis->get("zkb:corpKillmailScopesTotal"), false);
	...
	$balance = Util::formatIsk((float) $mdb->findField("payments", "balance", ['ref_type' => 'player_donation'], ['_id' => -1]));
...
function addInfo($text, $number, $left = true, $format = true)
{
	...
	$delta = (float) $number - (float) $prevNumber;
