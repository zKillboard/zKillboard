...
function calcStats($row)
{
	...
	{
		$isVictim = ($i == 0);
		if (($type == "regionID" || $type == "solarSystemID") && $isVictim == true) continue;
	...
	if ($r["ok"] != 1) die("stats update failure");
	if ($debug) Util::out("Stats completed for: $type $id $newSequence");
}
...
function mergeGroups(&$stats, $result, $isVictim)
{
	...
	$dl = ($isVictim ? "Lost" : "Destroyed");
	if (!isset($stats["groups"])) $stats["groups"] = [];
	$groups = $stats["groups"];
...
function mergeMonths(&$stats, $result, $isVictim)
{
	...
	$dl = ($isVictim ? "Lost" : "Destroyed");
	if (!isset($stats["months"])) $stats["months"] = [];
	$months = $stats["months"];
	foreach ($result as $row)
	{
		$year = $row["year"];
		...
		if (!isset($months[$yearMonth])) $months[$yearMonth] = [];
