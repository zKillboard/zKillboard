...
foreach($topics as $topic) $stomp->subscribe($topic, array("id" => "zkb-".$baseAddr, "persistent" => "true", "ack" => "client", "prefetch-count" => 1));
...
while ($timer->stop() <= 86400000)
{
	...
	if(!empty($frame))
	{
		...
		if (!$mdb->exists("apimails", ['killID' => $killID])) $mdb->insertUpdate("apimails", $killdata);
		...
		if (!$mdb->exists("stompmails", ['killID' => $killID])) $mdb->save("stompmails", $killdata);
		$stomp->ack($frame->headers["message-id"]);
	}
}
if ($stompCount > 0) Util::out("New kills from STOMP: $stompCount");
